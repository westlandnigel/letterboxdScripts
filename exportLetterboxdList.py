...
def extract_tmdb_info(movie_url):
    ...
    # Find the TMDb button (case-insensitive string match)
    tmdb_button = soup.find('a', class_='micro-button track-event', string=lambda s: s and 'tmdb' in s.lower())
    ...
    if tmdb_button:
        ...
        if "/tv/" in tmdb_link:
            media_type = "show"
        # Extract the last numeric ID at the end of the URL
        tmdb_id = tmdb_link.rstrip('/').split('/')[-1]
